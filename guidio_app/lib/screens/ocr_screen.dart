...
///
/// **Sepenuhnya on-device.** Alurnya jepret lalu `OcrService` (Google ML Kit)
/// lalu TTS. Tidak ada satu pun panggilan jaringan, dan karena itu tidak ada
/// satu pun kondisi jaringan yang boleh mematikan tombolnya.
///
/// Jalur `POST /api/ocr` sudah dihapus dari backend sejak pengenalan teks
/// dipindah ke ML Kit. Yang tertinggal di sini sampai sekarang cuma gerbang
/// UI-nya: tombol "Baca teks" dikunci saat offline dengan alasan "Butuh
/// internet untuk teks panjang". Itu bukan sekadar pesan usang, itu mematikan
/// fitur yang sebenarnya hidup - dan orang yang percaya lalu berhenti mencoba
/// justru saat label obat di tangannya paling perlu dibaca.
///
/// State yang butuh data yang tidak dihasilkan ML Kit (dua bahasa, sebagian
/// gagal, sangat panjang) dicapai lewat panel debug (lib/mock/ocr_mock_data.dart),
/// sesuai bagian 2 dokumen "boleh dipalsukan".
...
/// Kegagalan yang MASIH mungkin terjadi di mode ini.
///
/// `offline` dan `server` dihapus. Keduanya warisan saat OCR dikerjakan
/// backend lewat `POST /api/ocr`, endpoint yang sudah tidak ada. Membiarkan
/// keduanya berarti menyimpan cabang UI yang tidak pernah bisa dicapai, dan
/// setiap orang yang membacanya akan mengira mode ini masih butuh jaringan.
...
class _OcrScreenState extends State<OcrScreen> with WidgetsBindingObserver {
  ...
  void dispose() {
    ...
    // ponsel 2 GB setiap megabyte yang ditahan mode lain terasa di sana.
    //
    // Biayanya satu kali inisialisasi ulang saat mode ini dimasuki lagi,
    // bukan per pemindaian.
  ...
  Future<void> _scan() async {
    ...
    // sepenuhnya di perangkat lewat ML Kit, jadi "butuh internet"
  ...
  /// Banner mode ini TIDAK PERNAH menyebut server maupun internet.
  ///
  /// Pengenalan teks berjalan sepenuhnya di perangkat lewat ML Kit. Menyebut
  /// "tanpa internet" di sini bukan sekadar usang: ia mengajari pengguna bahwa
  /// fitur yang sebenarnya hidup itu sedang mati, dan orang yang percaya lalu
  /// berhenti mencoba justru saat label obat di tangannya paling perlu dibaca.
  Widget? _resolveBanner(bool storageLow) {
    ...
    if (_debugOverride == 'BT-05' || _nearTimeout) {
      return StatusBanner(tier: AlertTier.warning, message: 'Masih memproses, ${_elapsedSeconds}d…', actionLabel: 'Batalkan', onAction: () {
        _hardTimeoutTimer?.cancel();
        _elapsedTicker?.cancel();
  ...
  List<Widget> _buildContentZone(BuildContext context, double bottomInset) {
